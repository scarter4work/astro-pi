...
- **GitHub CLI (`gh`)**: Fully authenticated with full permissions (delete_repo, repo, workflow, etc.). Use this for all GitHub operations - creating repos, deleting repos, PRs, issues, etc.
- **MCP GitHub tools**: Available but `gh` CLI is often more flexible
- **Bash**: Full shell access
- **Render MCP**: For deployment operations

**Remember**: Check available tools before claiming something can't be done. The user knows what's available.
