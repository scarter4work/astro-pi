...
CompositorConfig NukeXInstance::BuildCompositorConfig() const
{
   ...
   // Segmentation - find and use available ONNX model
...
Image NukeXInstance::GeneratePreview( const Image& input, int previewMode ) const
{
   ...
   // For preview, use faster settings
   config.segmentationConfig.maxProcessingDimension = 512;  // Lower resolution for speed
   config.useParallelBlend = true;
   ...
   // Return based on preview mode
   switch ( previewMode )
   {
   case NXPreviewMode::RegionMap:
      if ( result.segmentationPreview )
         return *result.segmentationPreview;
      break;

   case NXPreviewMode::StretchedResult:
   default:
