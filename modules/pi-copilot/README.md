...
- **Requests** are non-streamed Messages API calls (models: ⚙ offers exactly **Claude Opus 5.5** `claude-opus-5-5` — the default — and **Claude Sonnet 5** `claude-sonnet-5`; a saved model that is no longer offered switches to Opus 5.5 with a one-time note in the chat log); the reply appears when complete, and the Send button reads "Thinking…" while a turn is in flight. Every text block of the reply is shown; a reply cut off by `max_tokens` ends with `[truncated: max_tokens]`. Streaming is a later increment.
