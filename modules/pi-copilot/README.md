...
- **What you read is what runs:** scripts containing invisible or text-reordering characters (zero-width and other format characters, bidi controls), control characters, or characters JavaScript treats as line breaks (U+2028/U+2029, a lone carriage return) are refused before you are asked; the model is told to write such characters as `\uXXXX` escapes. Windows line ends (CR LF) are accepted and shown and run as plain line feeds. The purpose line is limited to 300 characters.
